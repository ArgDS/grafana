...
export function describeTextRange(expr: any) {
  ...
  if (parts) {
    ...
    opt.display = 'от ' + opt.from + ' до ' + opt.to;
    opt.invalid = true;
  }
...
export function describeTimeRange(range) {
  ...
  if (moment.isMoment(range.from) && moment.isMoment(range.to)) {
    return 'от ' + formatDate(range.from) + ' до ' + formatDate(range.to);
  }

  if (moment.isMoment(range.from)) {
    var toMoment = dateMath.parse(range.to, true);
    return 'от ' + formatDate(range.from) + ' до ' + toMoment.fromNow();
  }

  if (moment.isMoment(range.to)) {
    var from = dateMath.parse(range.from, false);
    return 'от ' + from.fromNow() + ' до ' + formatDate(range.to);
  }
  ...
  return 'от ' + range.from.toString() + ' до ' + range.to.toString();
}
